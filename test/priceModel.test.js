...
describe("Price Model", () => {
  describe("Validation", () => {
    it("should validate price with invalid value", () => {
      ...
      expect(price.validateSync().errors.price.message).to.equal(
        'Cast to Number failed for value "abc" (type string) at path "price"',
      ); // Kiểm tra lỗi cast
    });
    ...
    it("should validate wholeSalePrice with positive value", () => {
      ...
      expect(price.validateSync().errors.wholeSalePrice.message).to.equal(
        "Whole sale price must be greater than 0",
      );
    });
    ...
    it("should validate retailPrice with positive value", () => {
      ...
      expect(price.validateSync().errors.retailPrice.message).to.equal(
    ...
    it("should validate promotionPrice with positive value", () => {
      ...
      expect(price.validateSync().errors.promotionPrice.message).to.equal(
    ...
    it("should validate importPrice with positive value", () => {
      ...
      expect(price.validateSync().errors.importPrice.message).to.equal(
        "Import price must be greater than 0",
      );
    });
  });
});
