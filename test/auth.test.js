...
describe('Authentication Middleware', () => {
  ...
  it('should return unauthorized if token is invalid', async () => {
    const mockVerify = sandbox.stub(jwt, 'verify').throws(new Error('Invalid token')); // Giả lập token không hợp lệ
    await checkToken(req, res, next);
    expect(mockVerify.calledOnceWith('token123', process.env.JWT_SECRET)).to.be.true;
    expect(res.status.calledOnceWith(HttpStatusCode.UNAUTHORIZED)).to.be.true;
    expect(res.json.calledOnceWith({ message: MESSAGE.TOKEN.INVALID })).to.be.true;
  });

  it('should return unauthorized if token is expired', async () => {
    const mockVerify = sandbox.stub(jwt, 'verify').throws({ name: 'TokenExpiredError' }); // Giả lập token hết hạn
    await checkToken(req, res, next);
    expect(res.status.calledOnceWith(HttpStatusCode.UNAUTHORIZED)).to.be.true;
    expect(res.json.calledOnceWith({ message: MESSAGE.TOKEN.EXPIRED })).to.be.true;
  });
