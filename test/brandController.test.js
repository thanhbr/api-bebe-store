...
describe("Brand Controller", () => {
  ...
  describe("getList", () => {
    ...
    it("should handle error when getting list of brands", async () => {
      ...
      expect(mockGetList.calledOnceWith({ search: "Nike", page: 1, size: 10 }))
        .to.be.true;
  ...
  describe("getDetail", () => {
    it("should return the details of a brand by ID", async () => {
      ...
      const mockGetDetail = sandbox
        .stub(brandRepository, "getDetail")
    ...
    it("should handle error when creating a new brand", async () => {
      ...
      expect(
        res.json.calledOnceWith({
          ...
          validateErrors: error.validationErrors,
        }),
      ).to.be.true;
    });
  });
